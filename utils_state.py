...
def get_last_incremental_value(state, API_name):
    """
    Obtiene el último valor incremental del JSON devuelto por la API en la ultima consulta (que es el mayor timestamp).

    Parámetros:
        state (dict): Objeto con el estado de la replicación
        API_name (str): Nombre de la API colocado en metadata_ingestion.json

    Retorna:
        Ultimo valor incremental del JSON devuelto por la API en la ultima consulta (El que figura en 
        metadata_ingestion.json). Debe ser date, datetime, timestamp.
    """
    try:
        return state[API_name]['last_value']
    except KeyError:
        raise KeyError(f"La API {API_name} no existe en el archivo JSON.")

def update_incremental_value(state, file_path, API_name, new_value):
    """
    Actualiza el valor incremental del JSON en el estado de la replicación
    ...
    last_incremental_value = get_last_incremental_value(state, API_name)

    # Chequeamos el tipo de datos
    if isinstance(new_value, datetime):  # Verificar primero si es datetime
        last_incremental_value = new_value
    elif isinstance(new_value, date):
        last_incremental_value = datetime.combine(new_value, datetime.min.time())
    else:
    ...
    state[API_name]['last_value'] = new_value
